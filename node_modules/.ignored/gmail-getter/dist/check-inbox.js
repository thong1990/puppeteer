 ...
 */
const checkInbox = async (options) => {
    ...
    while (Date.now() - startTime < timeout) {
        try {
            messages = await (0, api_1.fetchEmailsList)({ token, query });
        }
        catch (error) {
            console.log(error);
        }
    ...
    if (!all) {
        const email = await (0, api_1.fetchEmailById)(messages[0].id, token);
        return email;
    }
    let emails = [];
    for (const message of messages) {
        const email = await (0, api_1.fetchEmailById)(message.id, token);
        emails.push(email);
        await new Promise(resolve => setTimeout(resolve, step));
    }
    return emails;
